...
// FIX: clearCart se llamaba ANTES de window.location.href, lo que significaba que
//      si el redirect fallaba (ej: error de red), el carrito quedaba vacío sin que
//      el usuario haya pagado. Ahora clearCart se llama solo cuando hay URL de redirect
//      o cuando el fallback de éxito se activa.

import { useState, useEffect } from "react";
import { useCart } from "@/context/cart-context";
import { formatPrice } from "@/lib/utils";
import { Link } from "wouter";
import { ShieldCheck, ArrowLeft, CheckCircle2, Loader2, AlertTriangle, Clock, XCircle } from "lucide-react";
...
export default function Checkout() {
  ...
  // Todos los hooks ANTES de cualquier return condicional (regla de React Hooks)
  useEffect(() => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (url) {
        // FIX: clearCart SOLO cuando tenemos la URL de redirect confirmada.
        // Si falla antes de llegar aquí, el carrito se mantiene intacto.
        clearCart();
        window.location.href = url;
      } else {
        // Sandbox sin token real — simular éxito
        clearCart();
        window.location.search = "?status=success";
      }
    } catch (err: any) {
      // NO llamamos clearCart en el catch — el carrito queda para que el usuario reintente
